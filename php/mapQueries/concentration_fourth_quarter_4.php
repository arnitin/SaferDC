#!/usr/local/bin/php
<?php
$query = "select latitude A,longitude B,count(*) C from crime where latitude!=0 and longitude!=0 and to_number(to_char(report_date,'mm'))>9 and to_number(to_char(report_date,'mm'))<13 group by latitude,longitude order by count(*) desc";

/////////////////////////////////////////////////////////////Crime concentration third q
	/*select latitude,longitude,count(*)
	from crime
	where latitude!=0 and longitude!=0 and to_number(to_char(report_date,'mm'))>9 and to_number(to_char(report_date,'mm'))<13
