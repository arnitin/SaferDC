#!/usr/local/bin/php
<?php
$query = "select latitude A,longitude B,offence_type C,psa_id D from crime, (select aa.psa_id Thepsa,aa.A  from (select psa_id,count(*) A from crime  where psa_id>90 and psa_id <1000 group by psa_id) aa, (select min(count(*)) cnt from crime  where psa_id>90 and psa_id <1000 group by psa_id) B where B.cnt=aa.A) Qry where Qry.Thepsa=crime.psa_id";

/////////////////////////////////////////////////////////////lowest crime psa
/*


select latitude,longitude,offence_type,psa_id from crime, (select aa.psa_id Thepsa,aa.A  from (select psa_id,count(*) A from crime   group by psa_id) aa, (select min(count(*)) cnt from crime   group by psa_id) B where B.cnt=aa.A) Qry where Qry.Thepsa=crime.psa_id

select latitude A,longitude B,offence_type C,psa_id D
from crime,
